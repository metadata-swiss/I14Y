...
export class GeocatSearchResultsComponent extends SearchResultsComponent {
	...
	protected search() {
		...
		this.metasearchClient
			.getGeocatSearchByLanguageAndQueryAndPageAndPageSize(lang, query ?? '', page, pageSize)
			.pipe(first())
			.subscribe(response => {
