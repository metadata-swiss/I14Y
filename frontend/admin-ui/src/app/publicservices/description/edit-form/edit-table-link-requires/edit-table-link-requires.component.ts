...
@Component({
	selector: 'app-edit-table-link-requires',
	templateUrl: './edit-table-link-requires.component.html',
	styleUrls: ['./edit-table-link-requires.component.scss'],
	standalone: false
})
export class EditTableLinkRequiresComponent implements OnInit, OnDestroy, OnChanges {
	...
	private getDataSource(): void {
		if (this.publicService?.id) {
			this.publicServiceInputClient.getRequiresById(this.publicService.id).subscribe(response => {
