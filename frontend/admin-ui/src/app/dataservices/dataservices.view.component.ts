...
@Component({
	...
	templateUrl: './dataservices.view.component.html',
	styleUrls: ['./dataservices.view.component.scss'],
	standalone: false
})
export class DataservicesViewComponent implements OnInit, OnDestroy {
	...
	constructor() {
		...
		this.dataServiceId = this.route.snapshot.params.id;
	}

	ngOnInit(): void {
		this.route.params.pipe(takeUntil(this.unsubscribe$)).subscribe(params => {
			this.dataserviceService.load(params.id, true);
			this.dataServiceId = params.id;
		});
		this.dataserviceService.data$.pipe(takeUntil(this.unsubscribe$)).subscribe(x => (this.dataService = x));
		this.dataserviceService.registrationStatusInfo$.pipe(takeUntil(this.unsubscribe$)).subscribe(status => (this.registrationStatusInfo = status));
		this.dataserviceService.publicationLevelInfo$.pipe(takeUntil(this.unsubscribe$)).subscribe(level => (this.publicationLevelInfo = level));
		...
		this.allowActionService.load(this.dataServiceId, AllowActionResourceType.DataService);
	...
	onSetRegistrationStatus(status: RegistrationStatus) {
		this.dataServiceInputClient.putRegistrationStatusByIdAndStatus(this.dataServiceId, status).subscribe(() => {
			this.updateAfterSave();
		});
	}

	onProposeRegistrationStatus(proposal: RegistrationStatus) {
		this.dataServiceInputClient.putRegistrationStatusProposalByIdAndProposal(this.dataServiceId, proposal).subscribe(() => {
			this.updateAfterSave();
		});
	}

	onSetPublicationLevel(level: PublicationLevel) {
		this.dataServiceInputClient.putPublicationLevelByIdAndLevel(this.dataServiceId, level).subscribe(() => {
			this.updateAfterSave();
		});
	}

	onProposePublicationLevel(proposal: PublicationLevel) {
		this.dataServiceInputClient.putPublicationLevelProposalByIdAndProposal(this.dataServiceId, proposal).subscribe(() => {
			this.updateAfterSave();
		});
	}
	...
	onDeleteClick(): void {
		...
		this.translate.get([headertextKey, bodytextKey, confirmButtontextKey, DIALOG_CANCEL_BUTTON_KEY, DIALOG_CONFIRM_BUTTON_KEY]).subscribe(result => {
			...
			const dialogConfirm = dialogRef.componentInstance.confirm.subscribe(() => {
				this.dataServiceInputClient.deleteById(this.dataService?.id as string).subscribe(() => {
	...
	private updateAfterSave() {
		this.dataserviceService.load(this.dataServiceId, true);
		this.allowActionService.load(this.dataServiceId, AllowActionResourceType.DataService, true);
