...
internal static class DatasetMappingExtensions
{
        ...
        IVocabulariesService vocabulariesService)
    {
        ...
        return new()
        {
            ...
            ContactPoints = entity.ContactPoint.Select(x => x.MapToVCardModel()),
            DataOwner = entity.DataOwner,
            Description = entity.Description.MapToMultiLanguageModel(),
            Distributions = entity.Distributions.Select(x => x.MapToDcatDistributionModel(vocabulariesService)),
            Documentation = entity.Documentation.Select(x => x.MapToResourceModel()),
            Frequency = entity.Frequency?.MapToVocabularyEntryModel(frequencyTypesVocabulary),
            GeoIvIds = entity.GeoIvId.MapToVocabularyEntryModels(geoIvIdsVocabulary),
            Id = entity.Id,
            Identifiers = entity.Identifier,
            Images = entity.Image.Select(x => x.MapToResourceModel()),
            IsReferencedBy = entity.IsReferencedBy.Select(x => x.MapToResourceModel()),
            Issued = entity.Issued,
            Keywords = entity.Keyword.Select(x => x.MapToKeywordModel()),
            LandingPages = entity.LandingPage.Select(x => x.MapToResourceModel()),
            ...
            QualifiedAttributions = entity.QualifiedAttribution.Select(x => x.MapToDcatQualifiedAttributionModel(vocabulariesService)),
            QualifiedRelations = entity.QualifiedRelation.Select(x => x.MapToQualifiedRelationModel(relationshipRolesVocabulary)),
            RegistrationStatus = entity.RegistrationStatus,
            RegistrationStatusProposal = entity.RegistrationStatusProposal,
            Relations = entity.Relation.Select(x => x.MapToResourceModel()),
            ...
            TemporalCoverage = entity.TemporalCoverage.Select(x => x.MapToDateOnlyPeriodOfTimeModel()),
            Themes =  entity.Theme.MapToVocabularyEntryModels(themesVocabulary),
