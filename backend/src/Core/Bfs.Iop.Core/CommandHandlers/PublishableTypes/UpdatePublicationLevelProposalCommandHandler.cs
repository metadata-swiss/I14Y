...
internal sealed class UpdatePublicationLevelProposalCommandHandler : IRequestHandler<UpdatePublicationLevelProposalCommand>
{
    ...
    public UpdatePublicationLevelProposalCommandHandler(
    ...
    public async Task Handle(UpdatePublicationLevelProposalCommand request, CancellationToken cancellationToken)
    {
        var task = request.Type switch
        {
            PublishableResourceType.Dataset => _datasetsService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
            PublishableResourceType.PublicService => _publicServicesService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
            PublishableResourceType.DataService => _dataServicesService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
            PublishableResourceType.IopConcept => _iopConceptsService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
            PublishableResourceType.MappingTable => _mappingTablesService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
