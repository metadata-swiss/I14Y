...
{
    public static AgentModel MapToAgentModel(this Agent agent, IVocabularyReader vocabulariesService)
    {
        ...
        return new()
        {
            ...
            Images = agent.Images.Select(x => x.MapToResourceModel()),
            ...
            Spatial = agent.Spatial.AsEnumerable(),
            SpatialCH = agent.SpatialCH.Select(x => x.MapToVocabularyEntryModel(spatialCHVocabulary)),
            SubAgents = agent.SubAgents.Select(x => x.MapToIdNameModel()),
            Uid = agent.Uid,
        };
    }
