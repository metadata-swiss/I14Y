...
public sealed class ElasticsearchOptions
{
    ...
    /// <summary>
    /// When true, drop and recreate the index on startup before (re)building it.
    /// <para>
    /// Defaults to <c>false</c>: in a separately deployed service every container restart, revision
    /// rollout and scale event would otherwise wipe the search index and serve empty results until
    /// the rebuild finishes. Use DELETE /api/Index to rebuild deliberately.
    /// </para>
    /// </summary>
    public bool RecreateIndexOnStartup { get; set; }
}
