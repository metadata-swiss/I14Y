...
internal abstract class AbstractGeocatClient : IGeocatIndex
{
    ...
    public async Task<GeocatSearchResult> Search(string query, string culture, int? page, int? pageSize, CancellationToken cancellationToken)
    {
        ...
        var result = _mapper.Map<GeocatSearchResult>(
            response.Hits,
            opt => opt.Items["LinkBaseUri"] = $"{_baseUri}/{ConvertToGeocatCulture(culture)}/catalog.search#/metadata/"                    
        );
        result.From = from ?? 0;
