...
public class TypeScriptOperationNameGenerator : IOperationNameGenerator
{
    public bool SupportsMultipleClients { get; set; } = true;

    public string GetClientName(
        OpenApiDocument document, 
        string path,
        string httpMethod,
        OpenApiOperation operation) => GetClientName(path);
        ...
        OpenApiOperation operation)
    {
        ...
        var formattedHttpMethod = $"{httpMethod[..1].ToUpper()}{httpMethod[1..]}";
        ...
        {
            var prefix = index is 0 ? "By" : "And";
            builder.Append($"{prefix}{param.Name[..1].ToUpper()}{param.Name[1..]}");
            index++;
        }
    ...
    private static string GetClientName(string path)
    {
        ...
        ClientNameCollector.Instance.PushClientName(clientName);
