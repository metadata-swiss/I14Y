...
internal sealed class UpdatePublicationLevelProposalCommandHandler : IRequestHandler<UpdatePublicationLevelProposalCommand>
{
    ...
    public Task Handle(UpdatePublicationLevelProposalCommand request, CancellationToken cancellationToken)
    {
        var task = request.Type switch
        {
            PublishableType.Dataset => _datasetsService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
            PublishableType.PublicService => _publicServicesService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
            PublishableType.DataService => _dataServicesService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
            PublishableType.IopConcept => _iopConceptsService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
            PublishableType.MappingTable => _mappingTablesService.UpdatePublicationLevelProposal(request.Id, request.Proposal, cancellationToken),
