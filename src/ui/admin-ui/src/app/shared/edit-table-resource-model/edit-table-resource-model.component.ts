...
import {IResourceModel, MultiLanguage} from '@I14Y-ch/bfs-iop-admin-web-api-client';
import {URI_PATTERN} from 'src/app/app-constants';

@Component({
	selector: 'app-edit-table-resource-model',
	templateUrl: './edit-table-resource-model.component.html',
	styleUrls: ['./edit-table-resource-model.component.scss'],
	standalone: false
})
export class EditTableResourceModelComponent implements AfterViewInit, OnChanges {
	...
	public dataSource = new MatTableDataSource<IResourceModel>([]);
	...
	private initialRowValue: IResourceModel;
	private isEditing = false;
	private isAddMode = false;
	private rowIndex: number | undefined;
	private selection = new SelectionModel<IResourceModel>(true, []);
	...
	ngOnChanges(changes: SimpleChanges) {
		...
		if (change && change.currentValue !== undefined) {
			this.dataSource = new MatTableDataSource<IResourceModel>(this.dto);
	...
	onRemoveSelectedRows(): void {
		this.selection.selected.forEach(item => {
			const index: number = this.dataSource.data.findIndex((d: IResourceModel) => d === item);
			this.onRemoveRow(index);
		});
		this.selection = new SelectionModel<IResourceModel>(true, []);
	}
	...
	canSave(rowIndex: number): boolean {
		if (this.isRowEditMode(rowIndex) && !this.isControlInvalid(rowIndex)) {
			if (this.dataSource.data[rowIndex].uri) {
				return true;
			}
		}
		return false;
	...
	isRowSelected(row: IResourceModel): boolean {
	...
	onToggleSelection(row: IResourceModel): void {
		this.selection.toggle(row);
	}

	private copyRowValue(source: IResourceModel): IResourceModel {
		return {
			uri: source.uri,
	...
	private initResourceToTableData(): void {
		const formArray = new UntypedFormArray(
			this.dataSource.data.map(item => {
				if (item.label === undefined || item.label === null) {
					// initialize item.label to avoid modifidfication errors in case it is undefined
					item.label = new MultiLanguage({
						de: undefined,
						fr: undefined,
						it: undefined,
						en: undefined
				...
				return new UntypedFormGroup({
					uri: new UntypedFormControl(item.uri, [Validators.required, Validators.pattern(URI_PATTERN)]),
					label: new UntypedFormGroup({
		...
		this.parentForm.setControl(this.controlName, formArray);
	}

	private newEntry(): IResourceModel {
		return {
			uri: undefined,
			label: new MultiLanguage({
				de: undefined,
